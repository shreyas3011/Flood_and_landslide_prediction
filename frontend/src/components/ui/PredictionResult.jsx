...
import { Droplets, Mountain, ThermometerSun, Waves, Wind, Brain, AlertTriangle } from 'lucide-react';
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-3">
        ...
        <div className={`glass-card-blue p-4 rounded-2xl border-l-4 transition-all duration-300 hover:-translate-y-1 ${getRiskBorderClass(fp)}`}>
          ...
          <div className="flex items-start gap-1.5 text-xs text-slate-600 dark:text-slate-400 leading-snug">
            <div className="mt-0.5 shrink-0">{getRiskIcon(fp)}</div>
            <div>
              <span className="font-bold text-slate-800 dark:text-slate-200 mr-1">{getRiskLabel(fp)}:</span>
              {getRiskAdvice(fp)}
            </div>
          </div>
        </div>
        ...
        <div className={`glass-card-amber p-4 rounded-2xl border-l-4 transition-all duration-300 hover:-translate-y-1 ${getRiskBorderClass(lp)}`}>
          ...
          <div className="flex items-start gap-1.5 text-xs text-slate-600 dark:text-slate-400 leading-snug">
            ...
            <div>
              <span className="font-bold text-slate-800 dark:text-slate-200 mr-1">{getRiskLabel(lp)}:</span>
              {getRiskAdvice(lp)}
            </div>
          </div>
        </div>
      ...
      <div className="glass p-4 rounded-2xl shadow-xl relative overflow-hidden">
        ...
        <h4 className="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          {t('liveSatelliteFactors')}
        </h4>
        <div className="grid grid-cols-2 gap-2">
          <FactorItem icon={<Droplets size={13}/>} label={t('dailyRain')} value={`${lf.rainfall_mm} mm`} />
          <FactorItem icon={<Waves size={13}/>} label={t('sevenDayRain')} value={`${lf.antecedent_7day_mm} mm`} />
          <FactorItem icon={<Mountain size={13}/>} label={t('elevation')} value={`${lf.elevation_m} m`} />
          <FactorItem icon={<ThermometerSun size={13}/>} label={t('soilMoist')} value={`${(lf.soil_moisture * 100).toFixed(1)}%`} />
          <FactorItem icon={<Waves size={13}/>} label={t('discharge')} value={`${lf.river_discharge} m³/s`} />
          <FactorItem icon={<Wind size={13}/>} label={t('humidity')} value={`${lf.humidity_pct}%`} />
        </div>
      </div>
      ...
      {prediction.xai && (prediction.xai.flood_factors.length > 0 || prediction.xai.landslide_factors.length > 0) && (
        <div className="glass p-5 rounded-2xl border border-white/5 shadow-lg">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-semibold text-sm text-slate-800 dark:text-slate-200">{t('aiDecisionDrivers')}</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {prediction.xai.flood_factors.length > 0 && (
              <div>
                <h5 className="text-xs font-semibold text-blue-400 mb-3 uppercase tracking-wider">{t('floodFactorsTitle')}</h5>
                <div className="flex flex-col gap-2">
                  {prediction.xai.flood_factors.map((f, i) => (
                    <div key={i} className="flex justify-between items-center px-3 py-2 bg-blue-500/10 dark:bg-blue-500/10 rounded-lg text-xs border border-blue-500/10 transition-colors hover:bg-blue-500/20">
            ...
            {prediction.xai.landslide_factors.length > 0 && (
              <div>
                <h5 className="text-xs font-semibold text-amber-400 mb-3 uppercase tracking-wider">{t('landslideFactorsTitle')}</h5>
                <div className="flex flex-col gap-2">
      ...
      {(fp > 50 || lp > 50) && (
        <div className="mt-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <span className="font-extrabold text-sm text-red-400 uppercase tracking-[0.2em] drop-shadow-md">{t('highRiskDetectedTitle')}</span>
          </div>
...
function FactorItem({ icon, label, value }) {
  return (
    <div className="flex items-center gap-2 p-2 transition-all hover:bg-white/5 rounded-lg group/item cursor-default">
      <div className="w-6 h-6 flex-shrink-0 flex items-center justify-center rounded-md bg-slate-800 text-slate-600 dark:text-slate-400 group-hover/item:text-slate-800 dark:text-slate-200 group-hover/item:bg-slate-700 transition-colors">
        {icon}
      </div>
      <div className="flex flex-col min-w-0">
        <span className="text-[9px] font-semibold uppercase tracking-wider text-slate-500 truncate">{label}</span>
        <strong className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">{value}</strong>
      </div>
    </div>
  );
}
